    ...
    <main>
      ...
      <section className="relative overflow-hidden bg-black/80 px-3 md:px-6 py-20 text-white lg:py-28">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <Badge className="rounded-full p-3 bg-secondary text-sm md:text-base text-card hover:bg-secondary font-bold">Core service</Badge>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href="#book" className="inline-flex items-center justify-center rounded-xl bg-white/10 px-5 py-3 font-semibold text-white hover:bg-white/20 hover:transition-colors hover:ease-in-out duration-200">Request this service</a>
      ...
      <section className="bg-secondary px-6 py-16">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          <div><p className="text-sm font-bold uppercase tracking-widest text-primary">What is included</p><h2 className="mt-3 text-3xl font-black text-card">Everything under {service.name}</h2><p className="mt-4 text-sm md:text-base lg:text-lg leading-8 text-card/75">Choose the exact request type during booking so our team can recommend the right materials, production method, turnaround, and finishing path.</p></div>
          <div className="grid gap-4 sm:grid-cols-2">
            {service.subdivisions.map((item) => <div key={item.slug} className="rounded-2xl border border-primary/10 bg-card p-3 md:p-4 shadow-sm"><h3 className="font-bold text-card-foreground">{item.name}</h3>{item.children && <ul className="mt-3 space-y-1 text-sm text-muted-foreground">{item.children.map((child) => <li key={child.slug}>• {child.name}</li>)}</ul>}</div>)}
          </div>
        </div>
      </section>
