  ...
  try {
    ...
    const role = await transaction(async (c) => {
      const r = await c.query<any>(
        "update roles set name=$1,description=$2,is_super_admin=$3,updated_at=now() where id=$4 returning *",
        [data.name, data.description, data.isSuperAdmin, id],
      );
...
export async function DELETE(
  ...
  try {
    await requirePermission("ROLES_DELETE");
    const { id } = await params;
    const used = await query<{ count: string }>(
      "select count(*) from admins where role_id=$1",
      [id],
    );
    if (Number(used.rows[0]?.count) > 0)
      return NextResponse.json(
        { message: "Cannot delete a role assigned to admins" },
        { status: 409 },
      );
