...
export function ServiceBookingForm({ service }: { service: CoreService }) {
  ...
  return (
    <div
      id="book"
      className="rounded-[32px] border border-white/10 bg-card p-4 md:p-6 shadow-secondary shadow-sm md:p-8"
    >
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-bold font-manrope uppercase tracking-widest text-secondary">
            Book this service
          </p>
          <h2 className="mt-2 text-2xl font-black font-manrope tracking-wide text-card-foreground">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center justify-center rounded-xl bg-whatsapp px-4 py-2 font-semibold font-manrope text-white hover:bg-whatsapp/90"
        >
      ...
      <form className="grid gap-5" onSubmit={handleSubmit}>
        ...
        <div className="grid gap-5 md:grid-cols-2 *:text-black">
          ...
          <Input
            ...
            value={form.time}
            onChange={updateField}
            isToday={form.date === todayUTCDateString()}
          />
        </div>
        <Input
          name="projectName"
          className="text-black text-base tracking-wide"
        ...
        <Textarea
          name="description"
          className="text-black text-base tracking-wide"
          placeholder="Tell us about sizes, materials, deadline, artwork, and delivery needs..."
          rows={5}
        ...
        {status && (
          <p
            className="text-sm font-semibold font-inter text-card-foreground"
            aria-live="polite"
          >
            {status}
          </p>
