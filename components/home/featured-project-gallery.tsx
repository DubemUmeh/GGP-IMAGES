  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 transition-all duration-300 translate-y-0 opacity-100">
        ...
        <p className="mt-1 text-sm font-semibold font-manrope tracking-wide text-white">
          {p.title}
        </p>
      </div>
    </div>
...
export function FeaturedProjectsGallery() {
  return (
    ...
    // originally had.
    <section className="relative overflow-hidden bg-popover/80 pt-20 md:pt-28">
      ...
      <div className="mx-auto w-[min(100%,85rem)]">
        <div className="flex flex-wrap px-3 md:px-10 items-end justify-between gap-6 pl-3">
          <div>
            <Reveal>
              <div className="inline-flex items-center gap-2 rounded-full border border-background/15 bg-brand-tertiary px-4 py-2 text-xs font-semibold font-manrope uppercase tracking-widest text-background/70">
                Our Work
              </div>
            </Reveal>
            <Reveal delay={0.08} className="mt-6 max-w-xl">
              <h2 className="text-3xl font-extrabold font-manrope leading-tight tracking-wide text-foreground md:text-5xl">
                A few things we&apos;ve printed lately.
              </h2>
            </Reveal>
          </div>
          <Reveal delay={0.12}>
            <p className="max-w-sm md:max-w-md 2xl:max-w-lg text-sm lg:text-lg font-inter tracking-wider leading-relaxed text-foreground/80">
        ...
        <Reveal
          delay={0.18}
          className="mt-12 flex flex-col gap-6 bg-secondary p-5"
        >
