...
export function WorkProcess() {
  return (
    <section className="relative bg-inherit px-5 py-20 md:px-10 md:py-28">
      <div className={BG_GLOW_LIGHT} />
      <div className="mx-auto w-[min(100%,76rem)]">
        <Reveal>
          <div className="inline-flex items-center gap-2 rounded-full border border-border bg-brand-tertiary px-4 py-2 text-xs font-semibold font-manrope uppercase tracking-widest text-card shadow-[0_8px_20px_rgba(15,23,42,0.05)]">
            How It Works
          </div>
        </Reveal>
          ...
          {steps.map((s, i) => {
            ...
            return (
              <Reveal
                key={s.title}
                delay={i * 0.08}
                className="relative flex flex-col gap-4 border border-white/50 rounded-2xl bg-brand-tertiary z-50! p-4"
              >
                <div className="orange-glow relative z-10 flex h-12 w-12 items-center justify-center rounded-xl bg-white text-secondary font-bold font-manrope">
                  <Icon className="h-5 w-5 font-bold" />
                </div>
                <div className="flex items-baseline gap-2">
                  <span className="text-sm font-bold font-manrope text-popover">
                    0{i + 1}.
                  </span>
                  <h3 className="font-semibold font-manrope text-card tracking-wide">
                    {s.title}
                  </h3>
                </div>
                <p className="text-sm font-inter tracking-wide leading-relaxed text-card/80">
                  {s.desc}
                </p>
              </Reveal>
            );
