          ...
          {steps.map((s, i) => {
            ...
            return (
              <Reveal key={s.title} delay={i * 0.08} className="relative flex flex-col gap-4 border border-white/50 rounded-2xl bg-brand-tertiary z-50! p-4">
                <div className="orange-glow relative z-10 flex h-12 w-12 items-center justify-center rounded-xl bg-white/80 text-secondary font-bold font-manrope">
                  <Icon className="h-5 w-5" />
                </div>
                <div className="flex items-baseline gap-2">
                  <span className="text-sm font-bold font-manrope text-popover">0{i + 1}.</span>
                  <h3 className="font-semibold font-manrope text-card tracking-wide">{s.title}</h3>
                </div>
                <p className="text-sm font-inter tracking-wide leading-relaxed text-card/80">{s.desc}</p>
              </Reveal>
            );
          })}
        </div>
