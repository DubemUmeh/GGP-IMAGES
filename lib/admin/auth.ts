...
export async function currentAdmin(): Promise<AdminContext | null> {
  ...
  const res = await query<any>(
    `select a.id,a.email,a.name,a.avatar_url as "avatarUrl",r.id as role_id,r.name as role_name,r.is_super_admin, coalesce(array_agg(p.name) filter (where p.name is not null),'{}') as permissions from admins a join roles r on r.id=a.role_id left join role_permissions rp on rp.role_id=r.id left join permissions p on p.id=rp.permission_id where a.id=$1 and a.is_active=true group by a.id,r.id`,
    [id],
  );
...
export async function requirePermission(permission: PermissionName) {
  ...
  if (!admin.role.isSuperAdmin && !admin.permissions.includes(permission))
